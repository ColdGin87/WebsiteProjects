...
function isBirdieOrBetter(gross, par) {
  ...
  const p = Number(par);
  // Number(null) is 0 — an unscored teammate must never count as a birdie flip.
  if (!Number.isFinite(g) || !Number.isFinite(p) || g < 1) return false;
  return g <= p - 1;
}
...
function pairScores(players, scoring) {
  ...
  // First two rostered scores on this hole. Never a 1G+2N / vs-par team total.
...
function vegasGamesAtHole(presses, holeNumber) {
  let games = 1;
  for (const press of presses || []) {
    const key = press.game_key || press.gameKey;
    if (key && key !== 'vegas') continue;
    const start = Number(press.start_hole ?? press.startHole) || 1;
    if (start <= Number(holeNumber)) games += 1;
  }
  return games;
}

function scoreVegas({ holes, teams, scoring, dollarsPerPoint, startHole, endHole, presses } = {}) {
  ...
  for (const hole of holes || []) {
    ...
    let swingA = null;
    let swingB = null;
    const games = vegasGamesAtHole(presses, hn);
    if (!row.incomplete) {
      const posted = row.points * games;
      if (row.winner === 'A') {
        swingA = posted;
        swingB = -posted;
    ...
    holeRows.push({ holeNumber: hn, ...row, games, swingA, swingB, runA: signedA, runB: signedB });
  }
  const diff = signedA;
  const money = [];
  if (diff !== 0) {
  ...
  return {
    ...
    teamA: { id: sides[0].id, name: sides[0].displayName || sides[0].name, points: signedA },
    teamB: { id: sides[1].id, name: sides[1].displayName || sides[1].name, points: signedB },
    pointsA: signedA,
    pointsB: signedB,
    holes: holeRows,
    gamesRunning: vegasGamesAtHole(presses, end),
    money,
  };
}
