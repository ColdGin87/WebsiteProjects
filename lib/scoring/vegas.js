 ...
 * 10+ is written high-first. Birdie or better flips the opposing team (high first).
 * Both sides birdie → flips cancel. Net Vegas uses net numbers; flip only on gross birdie+.
...
function scoreVegasHole(teamA, teamB, { scoring, par } = {}) {
  ...
  const flipA = bBirdie && !aBirdie;
  const flipB = aBirdie && !bBirdie;
