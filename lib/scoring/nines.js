...
function scoreNines({ holes, members, scoring, blitz, dollarsPerPoint, startHole, endHole } = {}) {
  ...
  for (const hole of holes || []) {
    ...
    const pts = ninesHolePoints(keyed, useBlitz);
    if (!pts) {
      holeRows.push({ holeNumber: hn, incomplete: true });
      continue;
    }
    for (const player of three) {
      const add = pts[player.id] ?? pts[String(player.id)] ?? 0;
      totals.set(player.id, (totals.get(player.id) || 0) + add);
    }
    holeRows.push({ holeNumber: hn, incomplete: false, points: pts });
  }
  const nameOf = (id) => {
...
module.exports = { ninesHolePoints, scoreNines };
