 ...
 * Partnered = 1×. Lone after seeing drives = 2×. Blind lone (before others tee) = 3×.
...
function scoreWolfHole({ players, hole, pick, scoring }) {
  ...
  const multiplier = blind ? 3 : (lone ? 2 : 1);
  let winner = null;
...
function scoreWolf({ holes, members, picks, scoring, dollarsPerPoint, startHole, endHole } = {}) {
  ...
  for (const hole of holes || []) {
    ...
    const lone = !!(row.lone || row.blind);
    if (row.winner === 'wolf') {
      if (lone) {
        points.set(wolfId, (points.get(wolfId) || 0) + unit * fieldIds.length);
