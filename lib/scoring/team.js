/**
 * Goldendale team format: default lowest 1 gross + lowest 2 nets,
 * three different players. Dual-count optional.
 * Ties: lowest hole-total combination.
 * Fewer than needed scores: use what exists and flag incomplete.
 */
 ...
 * @param {{grossBalls?: number, netBalls?: number, dualCount?: boolean}} [settings]
...
function teamHoleScore(players, settings = {}) {
  ...
  const scored = (players || []).filter((p) => Number.isFinite(Number(p.gross)));
  ...
  if (scored.length === 0 || (grossBalls <= 0 && netBalls <= 0)) {
  ...
  for (const gPick of gCombos) {
    ...
    for (const nPick of nCombos) {
      const gTotal = gPick.reduce((s, p) => s + p.gross, 0);
      const nTotal = nPick.reduce((s, p) => s + p.net, 0);
