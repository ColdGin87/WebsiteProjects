...
const { sameTeamIds, canSeeTeammate, isFollowAlong } = require('../scoring/teamWrite');
...
function buildRoundState({ round, course, tee, holes, members, teams, scores, matches, presses, wolfPicks }) {
  ...
  for (const m of scoringOut) {
    const team = teams.find((t) => sameTeamIds(t.id, m.team_id ?? m.teamId));
    if (team && grouped.has(team.id)) grouped.get(team.id).members.push(m);
    else if (m.team_id && grouped.has(m.team_id)) grouped.get(m.team_id).members.push(m);
