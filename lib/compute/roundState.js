...
function computePlayingHandicap(handicap, tee, round) {
  if (handicap === null || handicap === undefined || handicap === '') return 0;
  if (!tee) return Number(handicap) || 0;
  return courseHandicap(handicap, {
    slope: tee.slope || 113,
    rating: tee.rating != null ? tee.rating : tee.par || 72,
    par: tee.par || 72,
    allowance: round.allowance || 100,
    holes: round.holes === '18' ? 18 : 9,
  });
}

function buildRoundState({ round, course, tee, holes, members, teams, scores, matches }) {
  const range = playingHoles(round);
  ...
  const holeResults = playHoles.map((hole) => {
    ...
    for (const team of grouped.values()) {
      ...
      const scored = teamHoleScore(playerScores, settings);
      perTeam.push({
        teamId: team.id,
        teamName: team.name,
        ...scored,
...
function resultsText(state) {
  ...
  lines.push(`Format: ${state.round.format === 'match_play' ? 'Match play' : '1 gross + 2 net team'}`);
  lines.push('');
  if (state.winner) {
    lines.push(`Winning team: ${state.winner.name} (${state.winner.total})`);
    lines.push('');
  }
  for (const team of state.teams) {
    lines.push(`${team.name}  ${team.total ?? '—'}  ${team.incomplete ? '(incomplete)' : ''}`);
...
function resultsCsv(state) {
  ...
  const allMembers = [
    ...
    ...state.unassigned.map((m) => ({ ...m, teamName: 'Individual' })),
  ];
