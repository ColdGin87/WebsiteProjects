...
const { sameTeamIds, canSeeTeammate, isFollowAlong, memberOnTeam } = require('../scoring/teamWrite');
...
function isShowOtherScoresOn(round) {
  if (!round) return false;
  return round.showOtherScores === true
    || round.show_other_scores === 1
    || round.show_other_scores === true;
}

function redactOtherTeamScores(state, viewerMember) {
  if (!state) return state;
  for (const member of state.members || []) {
    if (canSeeTeammate(viewerMember, member)) continue;
    for (const hole of member.holes || []) {
      hole.gross = null;
      hole.net = null;
    }
    member.outGross = null;
    member.inGross = null;
    member.totalGross = null;
    member.outNet = null;
    member.inNet = null;
    member.totalNet = null;
    member.holesPlayed = 0;
  }
  const myTeam = viewerMember && (viewerMember.team_id ?? viewerMember.teamId);
  for (const team of state.teams || []) {
    if (sameTeamIds(myTeam, team.id)) continue;
    team.total = null;
    team.out = null;
    team.inn = null;
    team.holes = (team.holes || []).map((h) => ({ ...h, total: null, balls: [] }));
  }
  for (const hr of state.holeResults || []) {
    hr.teams = (hr.teams || []).map((t) => (
      sameTeamIds(myTeam, t.teamId)
        ? t
        : { ...t, total: null, balls: [], incomplete: true }
    ));
  }
  if (state.winner && !sameTeamIds(myTeam, state.winner.id)) {
    state.winner = { ...state.winner, total: null };
  }
  return state;
}
...
function buildRoundState({ round, course, tee, holes, members, teams, scores, matches, presses, wolfPicks }) {
  ...
  const scoringOut = membersOut.filter((m) => !isFollowAlong(m));
  const teamRows = [];
  const grouped = new Map();
  for (const team of teams) grouped.set(team.id, { ...team, members: [] });
  const unassigned = [];
  for (const m of scoringOut) {
    let placed = false;
    for (const team of teams) {
      if (memberOnTeam(m, team.id) && grouped.has(team.id)) {
        grouped.get(team.id).members.push(m);
        placed = true;
      }
    }
  ...
  return {
    round: {
      ...
      showOtherScores: isShowOtherScoresOn(round),
    },
    holes: playHoles,
    members: membersOut,
    unassigned,
...
function livePatch(state, viewerMember) {
  const patch = {
    ...
    memberTotals: (state.members || []).map((m) => ({
      ...
      team_id: m.team_id,
      fill_team_id: m.fill_team_id ?? m.fillTeamId ?? null,
      fillTeamId: m.fill_team_id ?? m.fillTeamId ?? null,
      role: m.role,
      is_guest: m.is_guest,
      follow_show_other: m.follow_show_other,
      followShowOther: !!(m.followShowOther === true || m.follow_show_other === 1 || m.follow_show_other === true),
      outGross: m.outGross,
      inGross: m.inGross,
    ...
    teams: (state.teams || []).map((t) => ({
      ...
      name: t.name,
      nickname: t.nickname || '',
      displayName: t.displayName || teamDisplayName(t),
      total: t.total,
      out: t.out,
      inn: t.inn,
    ...
    winner: state.winner
      ...
      : null,
    showOtherScores: isShowOtherScoresOn(state.round),
  ...
  return redactLiveOtherTeamSetup(patch, viewerMember, isShowOtherScoresOn(state.round));
}

function redactLiveOtherTeamSetup(patch, viewerMember, showOther) {
  if (!patch || showOther || !viewerMember) return patch;
  const totals = (patch.memberTotals || []).map((m) => {
    if (canSeeTeammate(viewerMember, m)) return m;
    return { ...m, display_name: '' };
  });
  const teams = (patch.teams || []).map((t) => {
    if (memberOnTeam(viewerMember, t.id)) return t;
    return {
      ...t,
      holes: (t.holes || []).map((h) => ({ ...h, balls: [] })),
    };
  });
  return { ...patch, memberTotals: totals, teams };
}
...
function resultsText(state) {
  ...
  const formatLine = state.round.format === 'match_play'
    ? 'Match play'
    : (state.round.format === 'standard' || state.round.format === 'standard_scorecard')
      ? 'Standard scorecard'
