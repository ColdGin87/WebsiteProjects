...
function publicPlayer(player, viewer) {
  ...
  if (viewer && (viewer.is_admin || Number(viewer.id) === Number(player.id))) {
    out.email = player.email;
    out.is_admin = player.is_admin;
  }
  return out;
}
...
router.get('/', authenticate, async (req, res, next) => {
  try {
    ...
    res.json(players.map((p) => publicPlayer(p, req.user)));
  } catch (err) {
    next(err);
  }
});
