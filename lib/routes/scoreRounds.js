...
function slimScoreResponse(state, holeNumber) {
  return {
    ...
    sideGames: state.sideGames
      ? { stripText: state.sideGames.stripText, money: state.sideGames.money, config: state.sideGames.config }
      : null,
  };
}
    ...
    const result = await db.run(
      `INSERT INTO score_rounds
        (name, course_id, tee_id, format, holes, join_code, public_token, organizer_id,
         status, gross_balls, net_balls, dual_count, allowance, side_games)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'setup', ?, ?, ?, ?, ?)`,
      [
        name,
        courseId,
  ...
  try {
    ...
    await db.run(
      `UPDATE score_rounds SET name=?, format=?, holes=?, allowance=?, gross_balls=?, net_balls=?,
        dual_count=?, status=?, course_id=?, tee_id=?, side_games=? WHERE id=?`,
      [name, format, holes, allowance, grossBalls, netBalls, dualCount, status, courseId, teeId, sideGames, r.id]
    );
...
router.post('/:id/guests', authenticate, async (req, res, next) => {
  try {
    ...
    await db.run(
      ...
      [bundle.round.id, name, handicap, playing, bundle.round.tee_id, teamId == null ? null : teamId]
    );
  ...
  try {
    ...
    for (const g of guests) {
      ...
      if (g.playingHandicap != null || g.playing_handicap != null) {
        playing = Number(g.playingHandicap ?? g.playing_handicap);
      }
      const teamId = await resolveTeamId(db, bundle.round.id, g || {});
      await db.run(
        `INSERT INTO score_members
          (round_id, player_id, display_name, handicap, playing_handicap, tee_id, role, is_guest, team_id)
         VALUES (?, NULL, ?, ?, ?, ?, 'player', 1, ?)`,
...
async function applyDemoRoster(db, bundle, roster) {
  ...
  for (const player of roster) {
    ...
    if (taken && !taken.is_guest && taken.player_id) continue;
    ...
    const member = members.find((m) => m.display_name === player.name && m.is_guest);
    if (!member) continue;
...
router.put('/:id/wolf/:hole', authenticate, async (req, res, next) => {
  try {
    ...
    const lone = !!(req.body.lone || req.body.loneWolf);
    ...
    if (existing) {
      await db.run(
        'UPDATE score_wolf_picks SET wolf_member_id=?, partner_member_id=?, lone=? WHERE id=?',
        [wolfMemberId, partnerMemberId, lone ? 1 : 0, existing.id]
      );
    } else {
      await db.run(
        'INSERT INTO score_wolf_picks (round_id, hole_number, wolf_member_id, partner_member_id, lone) VALUES (?, ?, ?, ?, ?)',
        [bundle.round.id, holeNumber, wolfMemberId, partnerMemberId, lone ? 1 : 0]
      );
    }
    await touchRound(db, bundle.round.id);
    const next = await loadRoundBundle(db, bundle.round.id);
