...
const { autoBalanceTeams, sanitizeNickname, teamDisplayName, nextTeamLabel, joinTeamNickname } = require('../scoring');
const { parseSideGames, stringifySideGames, nassauSegmentForHole, segmentEnd, normalizeRoundFormat, isStandardScorecard, quietSideGames } = require('../scoring');
const { canWriteTeamScore, canAddGuestToTeam, canManageRosterMember, isFollowAlong, isFollowShowOtherOn, memberOnTeam, isGuestMember } = require('../scoring/teamWrite');
...
function parseJoinRole(body) {
  const raw = String((body && (body.role || body.joinRole || body.join_role)) || '').toLowerCase();
  if (raw === 'follower' || raw === 'follow' || raw === 'follow_along' || raw === 'follow-along') return 'follower';
  if (body && (body.followOnly === true || body.follow_only === true || body.followOnly === 'true')) return 'follower';
  return 'player';
}

function scoringMemberCount(members) {
  return (members || []).filter((m) => !isFollowAlong(m)).length;
}
...
function canScore(req, round, members, memberId) {
  ...
  const me = myRoundMember(req, members);
  return canWriteTeamScore(me, target, isOrganizer(req, round, members));
}

function canManageMember(req, round, members, target) {
  if (!req.user || !target) return false;
  if (!canSeeRound(req, round, members)) return false;
  const organizer = isOrganizer(req, round, members);
  return canManageRosterMember(myRoundMember(req, members), target, organizer);
}
...
function maskStateForViewer(state, req, members) {
  ...
  const me = myRoundMember(req, members);
  const hostAllows = isShowOtherScoresOn(state.round);
  if (isFollowAlong(me)) {
    if (hostAllows && isFollowShowOtherOn(me)) return state;
    return redactOtherTeamScores(state, me);
  }
  if (hostAllows) return state;
  return redactOtherTeamScores(state, me);
}
...
router.post('/', authenticate, async (req, res, next) => {
  try {
    ...
    const format = normalizeRoundFormat(req.body.format);
    ...
    let teamRace = req.body.teamRace === false || req.body.team_race === 0 || req.body.team_race === false ? 0 : 1;
    const showOtherScores = parseShowOtherScores(req.body.showOtherScores ?? req.body.show_other_scores, 0);
    let sideGames = stringifySideGames(req.body.sideGames || req.body.side_games);
    if (isStandardScorecard(format)) {
      teamRace = 0;
      sideGames = stringifySideGames(quietSideGames());
    }
...
router.post('/join', authenticate, joinRateLimit, async (req, res, next) => {
  try {
    ...
    if (!existing) {
      const roster = await db.all('SELECT role FROM score_members WHERE round_id = ?', [round.id]);
      const joinRole = parseJoinRole(req.body);
      if (joinRole !== 'follower' && scoringMemberCount(roster) >= 20) {
      ...
      const teamNick = joinTeamNickname(req.body);
      ...
      await db.run(
        `INSERT INTO score_members
          (round_id, player_id, display_name, handicap, playing_handicap, tee_id, role, is_guest, team_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)`,
        [round.id, user.id, cardName, user.handicap, playing, round.tee_id, joinRole, team && team.id]
      );
    }
...
router.get('/:id/live', authenticate, async (req, res, next) => {
  try {
    ...
    res.set('Cache-Control', 'no-store');
    res.json(livePatch(state, myRoundMember(req, bundle.members)));
  } catch (err) {
    next(err);
  }
});
...
router.put('/:id', authenticate, async (req, res, next) => {
  try {
    ...
    const format = req.body.format != null ? normalizeRoundFormat(req.body.format) : r.format;
    ...
    const teeId = req.body.teeId !== undefined || req.body.tee_id !== undefined
      ...
      : r.tee_id;
    let sideGames = req.body.sideGames != null || req.body.side_games != null
      ? stringifySideGames(req.body.sideGames ?? req.body.side_games)
      : (r.side_games || '{}');
    let teamRace = req.body.teamRace != null || req.body.team_race != null
...
router.put('/:id/follow-view', authenticate, async (req, res, next) => {
  try {
    const db = getDb();
    const bundle = await loadRoundBundle(db, req.params.id);
    if (!bundle) return res.status(404).json({ error: 'Round not found.' });
    ...
    if (!isFollowAlong(me)) {
      return res.status(403).json({ error: 'Follow along only. Scorekeepers use the host Show other teams setting.' });
    }
    const on = parseShowOtherScores(
      req.body.showOtherScores ?? req.body.followShowOther ?? req.body.follow_show_other,
      0
    );
    await db.run('UPDATE score_members SET follow_show_other = ? WHERE id = ? AND round_id = ?', [on, me.id, bundle.round.id]);
...
router.post('/:id/team-fill', authenticate, async (req, res, next) => {
  try {
    const db = getDb();
    const bundle = await loadRoundBundle(db, req.params.id);
    if (!bundle) return res.status(404).json({ error: 'Round not found.' });
    ...
    if (!isOrganizer(req, bundle.round, bundle.members)) {
      return res.status(403).json({ error: 'Only the host can accept a team fill spin.' });
    }
    const teamId = await resolveTeamId(db, bundle.round.id, req.body || {});
    if (teamId == null) {
      return res.status(400).json({ error: 'Pick the short team to fill.' });
    }
    const memberId = req.body.memberId ?? req.body.member_id;
    if (memberId != null && memberId !== '') {
      const member = bundle.members.find((m) => Number(m.id) === Number(memberId));
      if (!member) return res.status(404).json({ error: 'Player not found in this round.' });
      if (isFollowAlong(member)) {
        return res.status(400).json({ error: 'Follow along stays off the scoring roster.' });
      }
      if (memberOnTeam(member, teamId)) {
        return res.status(400).json({ error: 'That player is already on the target team.' });
      }
      const homeId = member.team_id ?? member.teamId;
      if (homeId == null || homeId === '') {
        await db.run('UPDATE score_members SET team_id = ? WHERE id = ? AND round_id = ?', [teamId, member.id, bundle.round.id]);
      } else {
        await db.run('UPDATE score_members SET fill_team_id = ? WHERE id = ? AND round_id = ?', [teamId, member.id, bundle.round.id]);
      }
    } else {
      const name = String(req.body.name || req.body.display_name || '').trim();
      if (!name) return res.status(400).json({ error: 'Pick a winner before Accept.' });
      if (scoringMemberCount(bundle.members) >= 20) {
        return res.status(400).json({ error: 'This round is full (20 players).' });
      }
      const handicap = req.body.handicap === undefined || req.body.handicap === '' ? null : String(req.body.handicap);
      const playing = computePlayingHandicap(handicap);
      await db.run(
        `INSERT INTO score_members
          (round_id, player_id, display_name, handicap, playing_handicap, tee_id, role, is_guest, team_id)
...
router.post('/:id/guests', authenticate, async (req, res, next) => {
  try {
    ...
    if (!organizer && isFollowAlong(me)) {
      return res.status(403).json({ error: 'Follow along cannot add players.' });
    }
...
router.put('/:id/members/:memberId', authenticate, async (req, res, next) => {
  try {
    ...
    if (!canManageMember(req, bundle.round, bundle.members, member)) {
...
router.delete('/:id/members/:memberId', authenticate, async (req, res, next) => {
  try {
    ...
    const organizers = (bundle.members || []).filter((m) => m.role === 'organizer');
    if (member.role === 'organizer' && organizers.length <= 1 && !isOrganizer(req, bundle.round, bundle.members)) {
      return res.status(403).json({ error: 'You cannot remove the organizer.' });
    }
    if (samePlayerId(member.player_id, req.user.id)) {
      return res.status(403).json({ error: 'You cannot remove yourself from the card. Clear guests instead.' });
    }
    await db.run('DELETE FROM score_holes WHERE member_id = ? AND round_id = ?', [member.id, bundle.round.id]);
    await db.run(
      'DELETE FROM score_matches WHERE round_id = ? AND (member1_id = ? OR member2_id = ? OR winner_member_id = ?)',
      [bundle.round.id, member.id, member.id, member.id]
    );
    await db.run(
      'DELETE FROM score_wolf_picks WHERE round_id = ? AND (wolf_member_id = ? OR partner_member_id = ?)',
      [bundle.round.id, member.id, member.id]
    );
    await db.run(
      'UPDATE score_presses SET pressed_by_member_id = NULL WHERE pressed_by_member_id = ? AND round_id = ?',
      [member.id, bundle.round.id]
    );
    await db.run('DELETE FROM score_members WHERE id = ? AND round_id = ?', [member.id, bundle.round.id]);
    await touchRound(db, bundle.round.id);
    const next = await loadRoundBundle(db, bundle.round.id);
    res.json(viewerRoundJson(next, req));
  } catch (err) {
...
router.post('/:id/clear-guests', authenticate, async (req, res, next) => {
  try {
    const db = getDb();
    const bundle = await loadRoundBundle(db, req.params.id);
    if (!bundle) return res.status(404).json({ error: 'Round not found.' });
    ...
    const me = myRoundMember(req, bundle.members);
    if (!me && !isOrganizer(req, bundle.round, bundle.members)) {
      return res.status(403).json({ error: 'Join this round before clearing guests.' });
    }
    if (isFollowAlong(me)) {
      return res.status(403).json({ error: 'Follow along cannot clear guests.' });
    }
    const guests = (bundle.members || []).filter((m) => isGuestMember(m) && canManageMember(req, bundle.round, bundle.members, m));
    for (const guest of guests) {
      await db.run('DELETE FROM score_holes WHERE member_id = ? AND round_id = ?', [guest.id, bundle.round.id]);
      await db.run(
        'DELETE FROM score_matches WHERE round_id = ? AND (member1_id = ? OR member2_id = ? OR winner_member_id = ?)',
        [bundle.round.id, guest.id, guest.id, guest.id]
      );
      await db.run(
        'DELETE FROM score_wolf_picks WHERE round_id = ? AND (wolf_member_id = ? OR partner_member_id = ?)',
        [bundle.round.id, guest.id, guest.id]
      );
      await db.run(
        'UPDATE score_presses SET pressed_by_member_id = NULL WHERE pressed_by_member_id = ? AND round_id = ?',
        [guest.id, bundle.round.id]
      );
      await db.run('DELETE FROM score_members WHERE id = ? AND round_id = ?', [guest.id, bundle.round.id]);
    }
    await touchRound(db, bundle.round.id);
    const next = await loadRoundBundle(db, bundle.round.id);
    res.json(viewerRoundJson(next, req));
...
router.post('/:id/scores', authenticate, scoreRateLimit, async (req, res, next) => {
  try {
    ...
    const me = myRoundMember(req, bundle.members);
    if (isFollowAlong(me)) {
      return res.status(403).json({ error: 'Follow along is read-only. You cannot enter scores.' });
    }
...
function canPress(req, round, members) {
  return canSeeRound(req, round, members);
}
...
router.post('/:id/presses', authenticate, async (req, res, next) => {
  try {
    ...
    const bundle = await loadRoundBundle(db, req.params.id);
    if (!bundle) return res.status(404).json({ error: 'Round not found.' });
    if (!canPress(req, bundle.round, bundle.members)) {
...
router.delete('/:id/presses/last', authenticate, async (req, res, next) => {
  try {
    const db = getDb();
    const bundle = await loadRoundBundle(db, req.params.id);
    if (!bundle) return res.status(404).json({ error: 'Round not found.' });
    ...
    const last = await db.get(
      'SELECT * FROM score_presses WHERE round_id = ? ORDER BY id DESC LIMIT 1',
      [bundle.round.id]
    );
    if (!last) return res.status(400).json({ error: 'No press to undo.' });
    await db.run('DELETE FROM score_presses WHERE id = ? AND round_id = ?', [last.id, bundle.round.id]);
    await touchRound(db, bundle.round.id);
    const next = await loadRoundBundle(db, bundle.round.id);
    res.json(viewerRoundJson(next, req));
  } catch (err) {
