...
const { canWriteTeamScore } = require('../scoring/teamWrite');
...
function wolfGameOn(round) {
  const cfg = parseSideGames(round && (round.side_games ?? round.sideGames));
...
function canScore(req, round, members, memberId) {
  ...
  if (canWriteTeamScore(me, target)) return true;
  // Wolf sides are not Team 1/2 — any rostered player may enter the Wolf card.
  if (wolfGameOn(round) && me) return true;
  return false;
}
...
router.post('/join', authenticate, async (req, res, next) => {
  try {
    ...
    if (!existing) {
      ...
      const cardName = String(req.body.displayName ?? req.body.display_name ?? req.body.playerName ?? user.name || '')
        .replace(/\s+/g, ' ')
        .trim()
        .slice(0, 40) || user.name;
      await db.run(
        `INSERT INTO score_members
          (round_id, player_id, display_name, handicap, playing_handicap, tee_id, role, is_guest, team_id)
         VALUES (?, ?, ?, ?, ?, ?, 'player', 0, ?)`,
...
router.get('/join-info', authenticate, async (req, res, next) => {
  try {
  ...
  try {
    ...
    if (gameKey === 'nassau') {
      ...
      if (segment === 'back' && startHole < 10) {
        return res.status(400).json({ error: 'Back press starts on holes 10–18.' });
      }
    }
