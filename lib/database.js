...
async function initDatabase() {
  await db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS score_members (
      ...
      fill_team_id INTEGER REFERENCES score_teams(id),
  ...
  await seedRounds();
  await wipePracticeScoreDataOnce();
}
...
async function migrateLegacyColumns() {
  ...
  await ensureColumn('score_rounds', 'show_other_scores', 'INTEGER DEFAULT 0');
  await db.exec(`
  ...
  await ensureColumn('score_teams', 'nickname', 'TEXT');
  await ensureColumn('score_members', 'follow_show_other', 'INTEGER DEFAULT 0');
  await ensureColumn('score_members', 'fill_team_id', 'INTEGER');
}
...
async function clearScorePlayData() {
  await db.exec(`
    DELETE FROM score_holes;
    DELETE FROM score_presses;
    DELETE FROM score_wolf_picks;
    DELETE FROM score_matches;
    DELETE FROM score_members;
    DELETE FROM score_teams;
    DELETE FROM score_rounds;
  `);
}

async function wipePracticeScoreDataOnce() {
  await db.exec(`
  ...
  `);
  const done = await db.get('SELECT value FROM app_meta WHERE key = ?', ['field_test_wipe']);
  if (done && String(done.value) === '20260907') {
    return { skipped: true, wiped: 0 };
  }
  const before = await db.get('SELECT COUNT(*) AS cnt FROM score_rounds');
  await clearScorePlayData();
  await db.run(
    `INSERT INTO app_meta (key, value, updated_at) VALUES (?, ?, datetime('now'))
     ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`,
    ['field_test_wipe', '20260907']
  );
  return { skipped: false, wiped: Number(before && before.cnt) || 0 };
}
