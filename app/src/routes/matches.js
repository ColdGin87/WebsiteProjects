...
leaderboardRouter.get('/', (req, res, next) => {
  try {
    ...
    const players = db.prepare('SELECT id, name, handicap FROM players ORDER BY handicap ASC').all();

    const leaderboard = players.map(player => {
      const wins = db.prepare(
        'SELECT COUNT(*) as count FROM matches WHERE winner_id = ? AND status = ?'
      ).get(player.id, 'completed');

      const total = db.prepare(
        `SELECT COUNT(*) as count FROM matches
         WHERE (player1_id = ? OR player2_id = ?) AND status = ?`
      ).get(player.id, player.id, 'completed');
...
router.get('/:id', (req, res, next) => {
  try {
    ...
    // Get stroke holes for this half
    ...
    ).all(match.course_id, holeRange[0], holeRange[1]);
    ...
    res.json({
      ...
      current_state: { score, holes_played: holes.length,
 ...
 */
router.put('/:id/score', authenticate, (req, res, next) => {
  try {
    const { db } = require('../../database/init');
    ...
    ).get(match.course_id, holeNumber);
    ...
    // Upsert
    const existing = db.prepare('SELECT id FROM match_holes WHERE match_id = ? AND hole_number = ?').get(match.id, holeNumber);
    if (existing) {
      db.prepare(`UPDATE match_holes SET player1_strokes=?, player2_strokes=?, player1_net=?, player2_net=?, hole_winner_id=? WHERE id=?`)
        .run(player1Strokes, player2Strokes, p1Net, p2Net, holeWinnerId, existing.id);
    } else {
      db.prepare(`INSERT INTO match_holes (match_id, hole_number, player1_strokes, player2_strokes, player1_net, player2_net, hole_winner_id) VALUES (?,?,?,?,?,?,?)`)
        .run(match.id, holeNumber, player1Strokes, player2Strokes, p1Net, p2Net, holeWinnerId);
    }

    if (match.status === 'pending') {
      db.prepare('UPDATE matches SET status = ? WHERE id = ?').run('active', match.id);
    }

    // Return updated state
    const allHoles = db.prepare('SELECT * FROM match_holes WHERE match_id = ? ORDER BY hole_number').all(match.id);
    let score = 0;
    allHoles.forEach(h => {
      if (h.hole_winner_id === match.player1_id) score++;
      else if (h.hole_winner_id === match.player2_id) score--;
    ...
    res.json({ match_id: match.id, hole: { hole_number: holeNumber, player1_strokes: player1Strokes, player2_strokes: player2Strokes, player1_net: p1Net, player2_net: p2Net, hole_winner_id: holeWinnerId, par: courseHole.par, stroke_index: courseHole.stroke_index }, match_state: { score, holes_played: allHoles.length } });
  } catch (err) {
    next(err);
  }
});
 ...
 */
router.put('/:id/finalize', authenticate, (req, res, next) => {
  try {
    const { db } = require('../../database/init');
    const match = db.prepare(`
    ...
    const holes = db.prepare('SELECT hole_number, hole_winner_id FROM match_holes WHERE match_id = ? ORDER BY hole_number').all(match.id);
    ...
    for (let i = 0; i < holes.length; i++) {
      ...
      const remaining = 9 - (i + 1); // 9-hole matches
    ...
    db.prepare('UPDATE matches SET status=?, winner_id=?, result_text=? WHERE id=?')
      .run('completed', winnerId, resultText, match.id);

    res.json({ match_id: match.id, status: 'completed', winner_id: winnerId, winner_name: winnerId === match.player1_id ? match.p1name : winnerId === match.player2_id ? match.p2name : null, result_text: resultText });
  } catch (err) {
    next(err);
  }
});
